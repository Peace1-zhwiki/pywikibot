...
cat_fa = pywikibot.Category(siteen,'Category:FA-Class physics articles')
cat_fl = pywikibot.Category(siteen,'Category:FL-Class physics articles')
cat_ga = pywikibot.Category(siteen,'Category:FA-Class physics articles')
cat_a = pywikibot.Category(siteen,'Category:A-Class physics articles')
cat_b = pywikibot.Category(siteen,'Category:B-Class physics articles')
cat_top = pywikibot.Category(siteen,'Category:Top-importance physics articles')
cat_high = pywikibot.Category(siteen,'Category:High-importance physics articles')
cat_mid = pywikibot.Category(siteen,'Category:Mid-importance physics articles')

page_to_write = pywikibot.Page(sitezh, u"User:和平奮鬥救地球/sandbox_3")
	...
	for page in listimp:
		...
		for ll in langlinks:
			...
			if(ll.site==sitezh):
				...
				break
		if zhleng>3000 or leng>20000: continue
